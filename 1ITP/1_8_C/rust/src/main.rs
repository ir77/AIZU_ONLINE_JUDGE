...
fn main() {
    ...
    let alphabet = (b'a'..b'z' + 1)
        ...
        .collect::<Vec<_>>();
    for lowercase in alphabet {
        let uppercase: char = lowercase.to_string().to_uppercase().chars().next().unwrap();
        let lowercase_count = inputs.match_indices(lowercase).count();
        let uppercase_count = inputs.match_indices(uppercase).count();
        println!("{} : {}", lowercase, lowercase_count + uppercase_count);
    }
}
